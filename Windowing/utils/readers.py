...
from utils.image import MammogramImage
...
def read_image_as_sitk_image(filename):
    ...
    try:
        ...
        dcm_image = pydicom.dcmread(filename)
        data = dcm_image.pixel_array
       
        if hasattr(dcm_image, 'PixelSpacing'):
            print(f"Pixel spacing from DICOM: {dcm_image.PixelSpacing}")
        else:
            dcm_image.PixelSpacing = "0.065238\\0.065238"
            print(f"Warning: DICOM file {filename} does not have PixelSpacing tag, using default spacing of (0.065238, 0.065238)")
        print(f"Pixel Spacing: {dcm_image.PixelSpacing}, Image shape: {data.shape}, Data type: {data.dtype}, Photometric Interpretation: {dcm_image.PhotometricInterpretation}")

        if dcm_image.PhotometricInterpretation == 'MONOCHROME1':
            # Pydicom loads the pixel data as is
            #data = dcm_image.pixel_array
            
            # Invert the image if needed
            data = data.max() - data 
            sitk_image = sitk.GetImageFromArray(data)
            #sitk_image.SetSpacing(tuple(np.append(dcm_image.ImagerPixelSpacing, 1)))
            sitk_image.SetSpacing(tuple(np.append(dcm_image.PixelSpacing, 1)))
                
            sitk_image.SetMetaData("PhotometricInterpretation", "MONOCHROME2")
            #sitk_image.SetPixelIDValue(sitk.sitkUInt16)  # Assuming the pixel data is 16-bit unsigned integer
        else:
            #sitk_image = sitk.ReadImage(str(filename))
            sitk_image = sitk.GetImageFromArray(data)
        
        #spacing = np.array(dcm_image.ImagerPixelSpacing)
        spacing = np.array(dcm_image.PixelSpacing)
